...
async function open(file, reopen = false) {
  const target = resolve(file);
  if (!existsSync(target)) throw new Error(`artifact not found: ${target}`);
  const existing = readRegistry(target);
  if (existing) {
    try {
      const healthRes = await requestTo(target, '/__kaya/health', { timeoutMs: 500 });
      // requestTo resolves to a Response, not a parsed body - read it before use.
      const health = await healthRes.json().catch(() => ({}));
      // A review the USER ended is theirs to resume. Refuse a plain reopen so an
      // agent cannot wander back into a session that was deliberately closed.
      // An agent ending its own turn does not lock the user out.
      if (health && health.endedBy === 'user' && !reopen) {
        console.error(`Kaya: ${basename(target)} was ended by you. Rerun with --reopen if you want to continue reviewing it.`);
        process.exitCode = 1;
        return;
      }
...
async function end(file) {
  const response = await requestTo(resolve(file), '/__kaya/end?by=agent', { method: 'POST' });
...
export async function main(args) {
  ...
  switch (args[0]) {
    ...
    case 'reopen': return open(args[1], true);
    ...
    default: return open(args[0], args.includes('--reopen'));
  }
}
