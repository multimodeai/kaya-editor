...
#kaya-convo .kaya-presence { margin-left:auto; display:inline-flex; align-items:center; gap:6px; font-size:10.5px; font-weight:700; letter-spacing:.06em; text-transform:uppercase; color:var(--muted); white-space:nowrap; }
#kaya-convo .kaya-presence::before { content:""; width:7px; height:7px; border-radius:50%; background:currentColor; flex:none; }
#kaya-convo .kaya-presence.kaya-live { color:#6fbf8b; }
#kaya-convo .kaya-presence.kaya-busy { color:var(--kt2); }
#kaya-convo .kaya-presence.kaya-off { color:#e0aa4e; }
#kaya-convo .kaya-nolisten { display:none; margin:10px 14px 0; padding:9px 12px; border:1px solid rgba(224,170,78,.4); border-radius:9px; background:rgba(224,170,78,.1); color:#e0aa4e; font-size:11.5px; line-height:1.45; }
#kaya-convo .kaya-nolisten.kaya-show { display:block; }
#kaya-convo .kaya-nolisten b { color:#f0c069; }
...
#kaya-overlay .kaya-ask,.kaya-ask{margin:14px 0;padding:14px 16px;border:1px solid rgba(199,91,63,.35);border-radius:10px;background:rgba(199,91,63,.06)}
.kaya-ask-label{font-size:13px;font-weight:600;margin-bottom:9px;opacity:.9}
.kaya-ask-row{display:flex;flex-wrap:wrap;gap:8px}
.kaya-ask-opt{font:inherit;font-size:13px;padding:7px 14px;border-radius:99px;cursor:pointer;
  border:1px solid rgba(127,127,127,.4);background:transparent;color:inherit;transition:background .12s,border-color .12s}
.kaya-ask-opt:hover{border-color:#c75b3f}
.kaya-ask-opt.kaya-on{background:#c75b3f;border-color:#c75b3f;color:#fff}

.kaya-pi-badge{display:inline-block;margin-left:8px;font-size:10px;font-weight:700;letter-spacing:.04em;
...
.kaya-overflowbtn:hover{background:rgba(224,170,78,.24)}
#kaya-zoombtn{position:fixed;z-index:2147483644;display:none;font:inherit;font-size:11px;font-weight:600;
  padding:4px 10px;border-radius:99px;cursor:pointer;border:1px solid rgba(255,255,255,.28);
  background:rgba(18,18,18,.82);color:#fff;backdrop-filter:blur(6px)}
#kaya-zoombtn:hover{background:#c75b3f;border-color:#c75b3f}
#kaya-lightbox{position:fixed;inset:0;z-index:2147483645;display:none;background:rgba(8,8,8,.94);
  cursor:grab;overflow:hidden}
#kaya-lightbox.kaya-show{display:block}
#kaya-lightbox.kaya-grabbing{cursor:grabbing}
.kaya-lb-stage{position:absolute;left:50%;top:50%;margin-left:-45vw;margin-top:-45vh;width:90vw;height:90vh;
  display:flex;align-items:center;justify-content:center;transform-origin:center center;will-change:transform}
.kaya-lb-stage>*{max-width:100%;max-height:100%;height:auto}
.kaya-lb-close{position:absolute;top:14px;right:16px;z-index:2;font:inherit;font-size:22px;line-height:1;
  width:36px;height:36px;border-radius:50%;cursor:pointer;border:1px solid rgba(255,255,255,.25);
  background:rgba(255,255,255,.08);color:#fff}
.kaya-lb-close:hover{background:#c75b3f;border-color:#c75b3f}
@media (prefers-reduced-motion: reduce){.kaya-lb-stage{will-change:auto}}

#kaya-lightbox.kaya-annotating{cursor:crosshair}
#kaya-lightbox.kaya-annotating .kaya-lb-stage *{cursor:crosshair}
#kaya-lightbox.kaya-annotating::after{content:'Annotate mode - click a node';position:fixed;left:50%;bottom:22px;
  transform:translateX(-50%);font:600 12px ui-sans-serif,system-ui;letter-spacing:.04em;padding:7px 14px;
  border-radius:99px;background:rgba(199,91,63,.92);color:#fff;pointer-events:none}
`;
